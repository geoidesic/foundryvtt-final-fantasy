  ...
  function filterQuery(data) {
    ...
    if (isIterable(properties)) {
      for (const property of properties) {
        if (regex.test(Strings.normalize(origin?.[property]))) { return true; }
      }
      return false;
    }
    else {
      // game.system.log.d('isNotIterable properties (i.e. just one filter)')
      // game.system.log.d(properties);
      // game.system.log.d(origin);
      // game.system.log.d(data);
